...
template<class tipo>bool Lista<tipo>::Extraer(Nodo<tipo>&x, int p)
{
	...
	else
	{
		if (p == -1)//eliminar el primero de la lista 
		{
			...
			p = _list;
			_list = _lista[_list].ApuntadorEst();//toma el valor del ApuntadorEst del valor que sale
			EliminarNodo(p);
			return true;
		}
		...
		{
			int y;
			if (_lista[p].ApuntadorEst() == -1)
			{
				return false;//si es que el p que mandan es el apuntodr del ultimo elemento 
			}
			else
			{
				x = _lista[_lista[p].ApuntadorEst()];
	
				y = _lista[p].ApuntadorEst();
				_lista[p].ApuntadorEst(_lista[_lista[p].ApuntadorEst()].ApuntadorEst());//hace que el anterior tome el ApuntadorEst delsiguiente
